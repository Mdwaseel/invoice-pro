...
import json
from utils.db import get_user_invoices, get_all_invoices
...
def show_invoices(supabase):
    ...
    filtered = invoices
    if search:
        filtered = [i for i in filtered if search.lower() in i.get("client_name","").lower() or search.lower() in i.get("invoice_number","").lower()]
    ...
    for inv in filtered:
        with st.expander(f"#{inv['invoice_number']} — {inv.get('client_name','N/A')} — ₹{inv.get('grand_total',0):.2f} — {inv.get('invoice_status','draft').upper()}"):
            ...
            with col3:
                new_status = st.selectbox("Status", ["draft","sent","paid"],
            ...
            if st.button("📄 View & Download PDF", key=f"pdf_{inv['id']}"):
                ...
                st.download_button("💾 Download PDF", data=buf.getvalue(),
                    file_name=f"{inv['invoice_number']}.pdf", mime="application/pdf",
                    key=f"dl_{inv['id']}")